...
with st.sidebar:
    ...
    st.markdown("##### Mercado & Macro")
    st.markdown("---")

    page = st.radio(
        "Navegação",
        ["🎯 Screener de Ações", "📈 Painel Macro", "📐 Renda Fixa", "🌍 Mercado Hoje"],
        label_visibility="collapsed",
    )

    st.markdown("---")
    st.markdown("[📋 CVM / CRE / Notícias →](https://investment-toolkit-2.streamlit.app)", unsafe_allow_html=True)
    st.markdown(
        "<p style='color:#6b7280; font-size:0.75rem;'>"
        "Dados: Fundamentus, BCB, ANBIMA, B3, FRED<br>"
